...
bool LoadPPMFile(uchar** data, int *width, int *height, PPM_LOADER_PIXEL_TYPE* pt, const char *filename)
{
	...
	char line[256];
  	char *ptr;
	tmp = fgets(line, 256, fid); //assigment to avoid warning
...
//    fgets(line, 256, fid);
  ...
  *height = strtol(ptr, &ptr, 10);
  if(ptr == NULL || *ptr == '\n'){
	...
	// Read pixel depth
  unsigned levels = strtol(ptr, &ptr, 10);
  unsigned pixel_depth = 0;
	...
	*data = new uchar[size];
	size_t tmp2 = fread(*data, 1, size, fid); //assigment to avoid warning
	fclose(fid);
	
	if(false && tmp2 != 0) ++tmp; //variable usage to avoid warning, this does nothing.
	return true;
}
