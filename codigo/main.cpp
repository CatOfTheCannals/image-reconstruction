...
void print_help(char* filename){
	cout << endl;
	cout << "uso: " << endl;
	cout << filename << " -i <input> -o <output> -n <n> [-r <ruido> -gX [params]]" << endl;
	cout << endl;
	cout << "donde: "		<< endl;
	...
	cout << "<n>\t"	<< "es el tamaño de la discretizacion. Debe ser un número natural multiplo del ancho y largo de <input>." << endl;
...
int main(int argc, char** argv){
	...
	if(n == 0 || input_img.size() == 0 || output_img.size() == 0){
	...
	if(geometrias.size() == 0){
		geometrias.push_back(make_pair("-g4", to_string(n*n*3))); //Default: geo random con el triple de rayos que de celdas.
	}

	try{

		Discretizador discretizacion(input_img, n, r);
		...
		for( auto geo : geometrias ){
			...
			if(num_geo == "-g1" ){
				...
				g5_params[0] = geo.second;
				//auto params = str_tokenize(geo.second, ",");
				if(g5_params[1] != ""){
					geo_punto_fijo_random pf(stoi(g5_params[0]), stoi(g5_params[1])); //ejemplo ./tp3 [...] -g5 5,6 -> llama pf(5, 6)
					pf.aplicar(discretizacion);
					g5_params[0] = "";
					g5_params[1] = "";
				}
